    ...
    <ProtectedRoute>
      <div className="flex">
        ...
        <main className="flex-1 ml-64 p-8 bg-gray-100 min-h-screen relative">
          ...
          {
            loading ?
              <div className='flex  items-center justify-center m-auto w-full h-full overflow-x-auto'><BiLoaderCircle size={30} className="animate-spin" /></div>
              :
              <div className='flex flex-col text-black p-8 rounded-lg gap-8'>
                <div>
                  {(resources.length > 0 ? (<div className="flex flex-col gap-5"> {
                    resources.map((item, index) => (
                      <div key={index} className="flex flex-col gap-2 border border-gray-500 p-5 rounded-md">
                        <div className="flex items-center text-sm gap-2">
                          <span className="bg-gray-200 py-1 px-2 rounded-2xl">{item.materialType}</span>
                          <span className="bg-transparent border border-gray-400 py-1 px-2 rounded-2xl">{item.level}</span>
                        </div>

...
export default function PairPage() {
  ...
  const handlePair = async () => {
    ...
    try {
      ...
      if (!Array.isArray(suggestions) || suggestions.length === 0) {
        setError("No pairing suggestions available");
        return;
      }
  ...
  const handleSendRequest = async () => {
    ...
    try {
      setIsPairing(true);
      const res = await sendPairingRequest(partner.id, user.course);
                ...
                <div className="text-center">
                  ...
                  <div className="flex items-center justify-center gap-8 mb-12">
                    ...
                    <div className="text-center">
                      <div
                        className={`w-20 h-20 ${
                          partner ? "bg-blue-500 text-white" : "bg-gray-300 text-gray-600"
                        } rounded-full flex items-center justify-center font-bold text-xl mb-3`}
                      >
                        {partner?.initials || "?"}
                      </div>
                      <p className="text-gray-700 font-semibold">
